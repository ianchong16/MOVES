...
// Assembled from: user profile + current filters + location + time.
...
struct MoveContext {
    ...
    let isWeekend: Bool
    let currentHour: Int       // Raw 0–23, for precise time-of-day safety filtering

    // History
    let recentMoveTitles: [String]

    // MARK: - Time-of-Day Safety Filter
    // Categories to exclude based on hour — prevents contextually inappropriate suggestions.
    var safetyExcludeCategories: [String] {
        var excluded: [String] = []

        switch currentHour {
        case 0..<6:
            // After 2am / before 6am: no outdoor parks or food spots
            excluded.append(contentsOf: ["park", "nature", "trail", "Park", "Nature", "restaurant", "food market"])
        case 6..<17:
            // Morning and afternoon: no nightlife or bars
            excluded.append(contentsOf: ["nightlife", "bar", "club", "lounge", "Nightlife", "speakeasy"])
        case 15..<17:
            // Dead zone 3–5pm: skip sit-down restaurant suggestions (too late for lunch, too early for dinner)
    ...
    var searchQueries: [String] {
        ...
        // Cap at 3 queries to keep API costs low
...
struct ContextBuilder {
    ...
    static func build(
        ...
        return MoveContext(
            ...
            filterSocialMode: socialMode.rawValue,
